...
#用于可视化滑动窗口位置和预测的车道线，并在二值化的透视变换图像上显示结果
...
#用于可视化带有边界的预测车道线，在二值化的透视变换图像上显示结果
def viz2(binary_warped, ret, save_file=None):
	...
	# Color in left and right line pixels  左右车道线颜色
	out_img[nonzeroy[left_lane_inds], nonzerox[left_lane_inds]] = [255, 0, 0]
	out_img[nonzeroy[right_lane_inds], nonzerox[right_lane_inds]] = [0, 0, 255]
...
def calc_curve(left_lane_inds, right_lane_inds, nonzerox, nonzeroy):
	...
	#根据拟合的曲线系数计算左侧和右侧车道线的曲率半径。这里使用的是曲率半径的计算公式，
	# 其中left_fit_cr[0]和right_fit_cr[0]是二次项的系数，left_fit_cr[1]和right_fit_cr[1]是一次项的系数
